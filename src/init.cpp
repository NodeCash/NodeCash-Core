...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: nodecash.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: nodecashd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizenodecashpamount=<n> " + _("Keep N NodeCash anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. NodeCash is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. NodeCash is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("NodeCash version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "NodeCash server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of NodeCash") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart NodeCash to complete") << "\n";
    ...
    nAnonymizeNodeCashAmount = GetArg("-anonymizenodecashamount", 0);
    if(nAnonymizeNodeCashAmount > 999999) nAnonymizeNodeCashAmount = 999999;
    if(nAnonymizeNodeCashAmount < 2) nAnonymizeNodeCashAmount = 2;
    ...
    LogPrintf("Anonymize NodeCash Amount %d\n", nAnonymizeNodeCashAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1NDH+1000 == (.1NDH+100)*10
       10NDH+10000 == (1NDH+1000)*10
    */

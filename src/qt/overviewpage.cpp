...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeNodeCashAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeNodeCashAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeNodeCashAmount = strAnonymizeNodeCashAmount.remove(strAnonymizeNodeCashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNodeCashAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeNodeCashAmount*COIN) nMaxToAnonymize = nAnonymizeNodeCashAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeNodeCashAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeNodeCashAmount));
        strAnonymizeNodeCashAmount = strAnonymizeNodeCashAmount.remove(strAnonymizeNodeCashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNodeCashAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {

...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeNodeCashAmount"))
        settings.setValue("nAnonymizeNodeCashAmount", 1000);
    nAnonymizeNodeCashAmount = settings.value("nAnonymizeNodeCashAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeNodeCashAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeNodeCashAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeNodeCashAmount:
            return QVariant(nAnonymizeNodeCashAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeNodeCashAmount:
            nAnonymizeNodeCashAmount = value.toInt();
            settings.setValue("nAnonymizeNodeCashAmount", nAnonymizeNodeCashAmount);
            emit AnonymizeNodeCashAmountChanged(nAnonymizeNodeCashAmount);
            break;
        default:
            break;
        }
